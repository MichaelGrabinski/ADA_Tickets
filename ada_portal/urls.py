...
# Only include permits URLs if the app is installed in this environment
if apps.is_installed('permits'):
    ada_patterns.append(path('permits/', include(('permits.urls', 'permits'), namespace='permits')))

urlpatterns = [
    # Azure AD / ADFS auth endpoints under /ADA/
    path('ADA/oauth2/', include('django_auth_adfs.urls')),
    path('ADA/', include(ada_patterns)),
]
